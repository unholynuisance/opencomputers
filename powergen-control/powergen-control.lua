...
local get_generator_to_enable = function(generators, grid_information)
    ...
    return table.max(generators, function(a, b)
        local a_info = grid_information[a.address]
        local b_info = grid_information[b.address]
...
local get_generator_to_disable = function(generators, grid_information)
    generators = table.vfilter(generators, function(v)
...
local main = function(config)
    ...
    while not should_stop do
        ...
        if 0 <= time_to_empty_at_max_output and time_to_empty_at_max_output < config.min_time_to_empty then
            local generator = get_generator_to_enable(generators)
            local generator_name = grid_information[generator.address].name
            print(string.format("Stopping generator %s", generator_name))
            generator.setWorkAllowed(true)
            lib.wait_for_stable_efficiency(generator)
        end

        if 0 <= time_to_full and time_to_full < config.min_time_to_full then
            local generator = get_generator_to_disable(generators)
            local generator_name = grid_information[generator.address].name
            print(string.format("Stopping generator %s", generator_name))
